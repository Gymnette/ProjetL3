...
def Question2(): 
    ...
    for degree in range(1, degreeMax+1) :
        ...
        (xnew,ynew) = AcquisitionOnePoint(ftest,a,b,"ob")
        # update of the DD
        xi,yi,delta = updateDD(xi,yi,delta,xnew,ynew)
        # evaluation of the interpolation polynomial
        NPoly *= (t - xi[degree-1])
        py += delta[degree] * NPoly 
        plt.plot(t, py, lw=1, label="degree = "+str(degree))   
    plt.legend(loc='best')
...
------------------------------------------------------"""
"""
    Modèle de coût : nombre de divisions + multiplications
    Ici, p = longueur de xi

    diffdiv:
        nombre d'opération : Cdiffdiv(p) = (p^2 - p) / 2 
    updateDD:
        nombre d'opérations : Cupdatedd(p) = 2*p 
    NewtonInterpol:
        nombre d'opérations (pour p > 2): Cnewtoninterpol(p) = Cdiffdiv(p) +  1 + (p-1)
                                                 = ((p^2 - p) / 2) + p
        Si p < 2:
            Cnewtoninterpol(p) = Cdiffdiv(p) + 1 = ((p^2 - p) / 2) + 1

    Coût de l'algorithme de Horner (question 1):
        Pour p points, donc un polynôme de degré p-1 :
        L'algorithme fait un appel simple à NewtonInterpol avec p points,
        pour un polynôme de degré n-1, on a donc un complexité de ((n^2 - n) / 2) + n si n > 2 sinon ((n^2 - n) / 2) + 1
    Coût de l'algorithme d'interpolation dynamique (question 2) :
        Pour n points, donc un polynôme de degré n-1:
        1 (diffdiv, car un seul élément dans xi) + (n-1)*(2*n +2) (boucle pour contenant la mise à jour des différences divisée)
        soit 2n^2 - 1
    Coût de l'algorithme en réappliquant Horner à chaque nouveau point ajouté:
        Pour n points, un polynôme de degré n - 1:
        pour n > 2:
            (n - 1) * (((n^2 - n) / 2) + n) = (n-1) * (n^2 + n)/2 = (n^3 - n)/2
        pour n <= 2:
            (n - 1) * (((n^2 - n) / 2) + 1) = (n^3 - 2n^2 +3n -2)/2
             
    Il n'est donc pas judicieux d'utiliser l'algorithme d'Horner dans le second cas car la complexité est bien plus grande.
	On a une complexité en O(n^3) avec cet algorithme, pour une complexité en O(n^2) en utilisant l'algorithme d'interpolation dynamique.
    L'intérêt de la base de Newton est de pouvoir ajouter des données sans tout recalculer : 
    il est donc absurde de tout recalculer lors de l'ajout d'une nouvelle donnée.
"""
