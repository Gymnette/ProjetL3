...
def HermiteC1(x0,y0,y0p,x1,y1,y1p):
    """
        # Interpolation cubique d'Hermite d'ordre 1 sur deux points x0 x1 avec x0 < x1 (interpolation et dérivée première) 
        input :  x0,y0,y0p,x1,y1,y1p - floats(Données de Hermite d'ordre 1(valeurs réelles))
        output : x, y - vecteurs de float(vecteurs d'ordonnées et d'abcisses de l'interpolation cubique d'Hermite)
    """
...
def MatriceS(n,h):
    """
        ...
        input : n - entier(nombre de neouds), h - float(pas régulier des noeuds sur l'intervalle du lissage [a,b])
        output : la matrice S
    """
    S=np.zeros((n-2,n-2))
    d=[2]
