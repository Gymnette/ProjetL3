...
def Question2(): 
    ...
    for degree in range(1, degreeMax+1) :
        ...
        NPoly *= [xt - xi[degree-1] for xt in t]
...
"""
    Coût de l'algorithme de Horner (question 1):
        Pour n+1 points, donc un polynôme de degré n :
        O(n) (boucle pour) + O(n^2) (fonction diffdiv) 
        = O(n^2)
    Coût de l'algorithme d'interpolation dynamique :
        Pour n+1 points, donc un polynôme de degré n :
        O(1) (diffdiv, car un seul élément dans xi) + O(n^2) (boucle pour contenant la différence divisée en O(n))
        = O(n^2)
    Dans le second cas, si on utilise l'algorithme de Horner, on aura dans la boucle pour :
        O(n) (de updateDD) + O(n^2) (algorithme d'Horner) = O(n^2)
        Ce qui donnera du O(n^3) au total, car il y a n tours de boucle.
    Il n'est donc pas judicieux d'utiliser l'algorithme d'Horner dans le second cas.
