...
# -*- coding: utf-8 -*-
"""
Created on Wed Oct  9 15:18:08 2019
...
def Affiche_Spline_NU(X,Y,a,b,n,label="",color='r'):
    '''
...
def Affiche_Spline_3D(a,b,X,Y,Z,label="",color="r",type_repartition = "",nouvelle_figure = True):
    '''
    Affichage de la courbe paramétrique spline interpolant les points dans l'intervalle [a,b],
    sur n points d'interpolation répartis uniformément, façon chebyshev ou chordale
    ...
    #Spline des (ti,zi)
    A = Matrix_NU(H)
    B = Matrix_NU_resulat(Z,H)
    Zp = np.linalg.solve(A,B)
    Sz = []
    for i in range(0,n-1):
        Sz.append(HermiteC1_non_affiche(T[i],Z[i],float(Zp[i]),T[i+1],Z[i+1],float(Zp[i+1])))
    #affichage
    plot.plot1d2d(Sx[0][1], Sy[0][1], Sz[0][1], legend=label, new_fig=nouvelle_figure, show=False,color = color,title = "")
    for i in range(1,n-1):
        plot.plot1d2d(Sx[i][1], Sy[i][1], Sz[i][1], legend="", new_fig=False, show=False,color = color,title = "")
        #plt.plot(Sx[i][1],Sy[i][1],Sz[i][1],color)
...
def load_points_3D(fichier):
    (X,Y,Z) = np.loadtxt(fichier)
    return X,Y,Z
    
def sortpoints(X,Y,Z = None):
    """
    ...
    if Z != None :
        ...
        for i in range(n):
            Dz[X[i]] = Z[i]
    Xb.sort()
    Yb = [Dy[Xb[i]] for i in range(n)]
    if Z != None:
        Zb = [Dz[Xb[i]] for i in range(n)]
    return Xb,Yb,Zb
...
if __name__ == "__main__":
    ...
    if mode == '4' :
        U,V,W = load_points_3D('test3D.txt')
        a = min(U)
        b = max(U)
        n = len(U)
        
        Affiche_Spline_3D(a,b,U,V,W,label="pas de tri",color="r")
        X,Y,Z = sortpoints(U,V,W)
        Affiche_Spline_3D(a,b,U,Z,W,label="tri sur X",color="b",nouvelle_figure=False)
