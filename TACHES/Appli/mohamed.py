...
# AJOUTE POUR LE CODE DE MOHAMED
...
if __name__ == "__main__":  
    ...
    # uk, zk = ldt.sortpoints(uk, zk)
    uk, zk, f1, _ = non_stationary_signal((30,), 0.9, noise_func=nfunc)
    
    #uk,zk = ldt.sortpoints(uk,zk)

    a = min(uk) # intervalle
    b = max(uk) # intervalle
    n= 5 # nombre des noeuds attendus pour la spline de lissage
    plt.figure()
    plt.title('spline de lissage avec '+str(n)+' noeuds') # titre
    plt.plot(uk,zk,"+")
    uk = list(uk)
    zk = list(zk)
    while True: #On break cette boucle lorsqu'il n'y a plus de points aberrants
        ...
        ind_le_plus_aberrant = erreur.Erreur(uk,zk,xx,yy,seuil)
