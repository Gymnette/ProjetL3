...
def scatterdata(X, Y, c = 'r+', legend="", title="", new_fig=True, show=True, xlabel=None, ylabel=None):
    if new_fig:
        plt.figure(title)

    plt.title(title)
    plt.plot(X, Y, c, label=legend)
...
def plot2d1d(X, Y, fz, c = 'b', title = "", show=True, show_scale =True):
    """
...
def plotlevel(X, Y, fz, tab_levels=None, c='b', title="", new_fig=True, show=True):
    """
    ...
    contours = plt.contour(X, Y, Z,c=c, levels=tab_levels)
